...
describe('Card tests', async () => {
    screen.debug();
    const result = {title: 'aaa', author_name: 'aaa', first_publish_year: '1111', key: 'aaa'};
    it('Card component renders renders the relevant card data', async () => {
        render(<ResultsUnit title={result.title} key={result.key} first_publish_year={result.first_publish_year}
                            author_name={result.author_name} onClick={() => Object}/>)
        ...
        expect((a.children[1] as HTMLDivElement).innerHTML).toContain(result.author_name);
        expect(a.children[2].className).toEqual("info results_first-published");
        expect((a.children[2] as HTMLDivElement).innerHTML).toContain(result.first_publish_year);
    });
});
