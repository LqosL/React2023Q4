...
export default function Page(
  ...
  const [loaderMain, setLoaderMain]: [
    loaderMain: boolean,
    setLoaderMain: React.Dispatch<React.SetStateAction<boolean>>,
  ] = useState(false);

  const [currentDetailKey, setCurrentDetailKey]: [
    currentDetails: string | undefined,
    setCurrentDetails: React.Dispatch<React.SetStateAction<string | undefined>>,
  ] = useState(pageState.details?.details?.key);

  const [recentState, setRecentState]: [
    recentState: { docs: Array<Result> },
    setRecentState: React.Dispatch<
      React.SetStateAction<{ docs: Array<Result> }>
    >,
  ...
  function closeDetails() {
    ...
    setTimeout(() => {
      router.push(
        urlBuilder(undefined, pageState.page, itemsPerPage, searchInputState)
      );
    }, 1);
  }
