...
describe('It correctly displays the detailed card data', () => {
    ...
    it('displays the detailed card data', async () => {
        ...
        expect(a.children[3].innerHTML).contains('https://openlibrary.org' + details.key);
        expect(a.children[4].innerHTML).contains('Open Library Author link:');
        expect(a.children[4].innerHTML).contains('https://openlibrary.org' + details.authors[0].author.key);
    });
});
        ...
        render(
            <MemoryRouter initialEntries={['/test']}>
                <Routes>
                    ...
                    <Route path={'/test'} element={<SectionDetailsContainer key={details.key}/>}></Route>
                </Routes>
            </MemoryRouter>
        )
